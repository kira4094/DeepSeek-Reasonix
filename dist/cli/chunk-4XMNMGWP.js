  ...
  "node_modules/domutils/lib/helpers.js"(exports) {
    ...
    function uniqueSort(nodes) {
      ...
      nodes.sort(function(a, b) {
        var relative9 = compareDocumentPosition(a, b);
...
function registerSingleMcpTool(mcpTool, env) {
  ...
  env.registry.register({
    ...
    parameters: stableTool.inputSchema,
    fn: async (args, ctx) => {
      if (env.ready) {
        await waitForReady(
          env.ready,
...
async function bridgeMcpTools(client, opts = {}) {
  ...
  const stableTools = listed.tools.map((tool) => canonicalizeMcpToolForCache(tool)).sort((a, b) => `${prefix}${a.name}`.localeCompare(`${prefix}${b.name}`));
...
function canonicalizeMcpToolForCache(tool) {
  return {
    ...tool,
    inputSchema: canonicalizeSchemaForCache(tool.inputSchema)
  };
}
var SET_LIKE_SCHEMA_ARRAY_KEYS = /* @__PURE__ */ new Set(["required", "dependentRequired"]);
function canonicalizeSchemaForCache(value, parentKey) {
  ...
  if (parentKey === "dependentRequired") {
    ...
    for (const key of Object.keys(value).sort()) {
      const arr = value[key];
      out2[key] = Array.isArray(arr) && arr.every(isScalar) ? [...arr].sort((a, b) => String(a).localeCompare(String(b))) : canonicalizeSchemaForCache(arr, key);
    }
    return out2;
  }
  const out = {};
  for (const key of Object.keys(value).sort()) {
    out[key] = canonicalizeSchemaForCache(value[key], key);
  }
  return out;
}
...
function plainTextRejectedReason(name, result) {
  ...
  if ((name === "edit_file" || name === "write_file" || name === "multi_edit" || name === "delete_range" || name === "delete_symbol") && /queued \d+ edits? for review/i.test(result)) {
    return "edit-gate";
  }
  if ((name === "edit_file" || name === "multi_edit" || name === "delete_range" || name === "delete_symbol") && /read_file first/i.test(result)) {
...
var ImmutablePrefix = class {
  ...
  tools() {
    ...
    const frozen = Object.freeze(
      this._toolSpecs.map((t2) => Object.freeze({ ...t2, function: { ...t2.function } }))
    );
    this._frozenToolsCache = frozen;
  ...
  diagnosticHashes(toolSpecs = this.tools()) {
  ...
  invalidatePrefixCaches() {
    ...
    this._diagnosticHashesCache = /* @__PURE__ */ new WeakMap();
  }
  computeDiagnosticHashes(toolSpecs) {
...
var AppendOnlyLog = class {
  ...
  initWindow(messages) {
    this._entries = messages.length > this._windowSize ? messages.slice(messages.length - this._windowSize) : [...messages];
  ...
  getEntry(index) {
    if (index < 0 || index >= this._totalLength) return void 0;
...
function parseNeedsProEscalation(content) {
...
var CacheFirstLoop = class _CacheFirstLoop {
  client;
  prefix;
  tools;
  log;
  ...
  static DEFAULT_MAX_ITER_PER_TURN = 50;
  ...
  constructor(opts) {
    ...
    this.log = new AppendOnlyLog({
      sessionPath: this.sessionName ? sessionPath(this.sessionName) : void 0
    });
    ...
    this.maxIterPerTurn = opts.maxIterPerTurn ?? _CacheFirstLoop.DEFAULT_MAX_ITER_PER_TURN;
...
`
  ...
  configure(opts) {
    ...
    if (opts.maxOutputTokens !== void 0) {
      this.maxOutputTokens = opts.maxOutputTokens ?? void 0;
    }
  }
...
function listFilesWithStatsSync(root, opts = {}) {
  ...
  const walk2 = (dirAbs, dirRel, layers) => {
    ...
    for (const ent of entries) {
      ...
      const absPath = join2(dirAbs, ent.name);
      if (ent.isDirectory()) {
...
async function flushFiles(ents, dirAbs, dirRel, layers, emit) {
  const accepted = ents.filter((e) => !ignoredByLayers(layers, join2(dirAbs, e.name), false));
  const stats = await Promise.all(
    accepted.map(
      (e) => stat(join2(dirAbs, e.name)).then((s) => ({ mtimeMs: s.mtimeMs, isFile: s.isFile() })).catch(() => null)
    )
  );
...
var BING_INTL_ENDPOINT = "https://www.bing.com/search";
var METASO_ENDPOINT = "https://metaso.cn/api/v1";
var BAIDU_AI_SEARCH_ENDPOINT = "https://qianfan.baidubce.com/v2/ai_search/web_search";
...
function searchSignal(callerSignal) {
  const t2 = AbortSignal.timeout(SEARCH_TIMEOUT_MS);
  return callerSignal ? AbortSignal.any([callerSignal, t2]) : t2;
}
async function searchBing(query, opts = {}, endpoint = BING_ENDPOINT) {
...
async function searchBaidu(query, opts = {}) {
  const topK = Math.max(1, Math.min(10, opts.topK ?? DEFAULT_TOPK));
  ...
  return (data.references ?? []).filter((r) => typeof r.title === "string" && typeof r.url === "string").slice(0, topK).map((r) => ({
    title: r.title,
    url: r.url,
...
// src/code/auto-git-rollback.ts
import { execFileSync } from "child_process";
import { existsSync as existsSync3, realpathSync } from "fs";
import { basename, dirname, isAbsolute as isAbsolute2, join as join3, relative as relative3, resolve as resolve4 } from "path";
function rejection(message, nextAction) {
  return JSON.stringify({
...
function autoGitRollbackActive(rootDir, autoGitRollback) {
...
function targetExistsOrIsTracked(topLevel, relPath) {
  if (existsSync3(join3(topLevel, relPath))) return true;
  return tryGit(topLevel, ["ls-files", "--error-unmatch", "--", relPath]) !== null;
}
function realpathForTarget(abs) {
  const resolved = resolve4(abs);
  if (existsSync3(resolved)) return realpathNative(resolved);
  const missing = [basename(resolved)];
  let parent = dirname(resolved);
  while (!existsSync3(parent)) {
    const next = dirname(parent);
    if (next === parent) return resolved;
    missing.unshift(basename(parent));
    parent = next;
  }
  return join3(realpathNative(parent), ...missing);
}
function commitMessage(toolName, relPaths) {
...
function normalizeGitRelPath(path) {
  return path.replaceAll("\\", "/");
}
function parseStatusPath(out) {
  if (!out) return null;
...
function gitRelPathForTarget(topLevel, absPath) {
...
function prepareAutoGitRollback(opts) {
  ...
  if (stageable.length > 0) {
    try {
      ...
      return rejection(err.message, "fix_git_add_failure");
    }
  }
  ...
  if (hasStagedChanges) {
    try {
      ...
      return rejection(err.message, "fix_git_commit_failure");
    }
  }
...
function looksLikeAbsoluteSystemPath(rawPath) {
...
function resolveBlockPath(rootDir, rawPath) {
  ...
  return resolve4(rootDir, rooted || ".");
}
function prepareAutoGitRollbackForEditBlocks(rootDir, blocks, autoGitRollback) {
...
function formatAutoGitRollbackRejection(result) {
  try {
    const parsed = JSON.parse(result);
...
import { dirname as dirname2, join as join4, relative as relative4, resolve as resolve5 } from "path";
...
function findSubdirMemoryAncestors(absPath, rootDir) {
  return findDirMemory(dirname2(resolve5(absPath)), rootDir);
}
...
var RegexRunner = class {
  ...
  testLines(text, source, flags, opts = {}) {
    return new Promise((resolve8, reject) => {
      ...
      const entry = { resolve: resolve8, reject, timer };
      if (opts.signal) {
...
var DELETE_SYMBOL_KINDS = /* @__PURE__ */ new Set([
  "function",
  "class",
  "method",
  "interface",
...
function looksLikeAbsoluteSystemPath2(raw) {
...
${body}`;
  ...
  async function ensureOutsideSandboxAllowed(abs, intent, toolName, ctx) {
    for (const dir of loadProjectPathAllowed(rootDir)) {
      if (pathIsUnder2(abs, dir)) return;
    }
  ...
  const safePath = async (raw, toolName, ctx, intent = "read") => {
    ...
    if (looksLikeAbsoluteSystemPath2(raw)) {
      const abs = pathMod5.resolve(raw);
      if (pathIsUnder2(abs, normRoot)) return abs;
    ...
    if (!pathIsUnder2(resolved, normRoot)) {
      throw new Error(
...
` : "";
  ...
  registry.register({
    ...
    fn: async (args, ctx) => {
      ...
      let oldContent = null;
      try {
        const buf = await fs3.readFile(abs);
  ...
  registry.register({
    ...
    fn: async (args, ctx) => {
      ...
      return applyEdit(
        ...
        abs,
        args,
        ctx?.readTracker ? (abs2) => ctx.readTracker.hasRead(abs2) : void 0
      );
    }
  });
  registry.register({
  ...
  registry.register({
    name: "delete_range",
    description: "Delete a contiguous text range from an existing file using exact start/end anchors. Call read_file on this path first this session. Matching failures, duplicate anchors, or reversed ranges are no-ops and leave the file unchanged. Use this instead of huge SEARCH/REPLACE blocks for large deletions.",
    parameters: {
      type: "object",
      properties: {
        path: { type: "string" },
        ...
        inclusive: {
          type: "boolean",
          description: "When true (default), delete the anchors too. When false, keep both anchors and delete only the text between them."
        }
      },
  ...
  registry.register({
    name: "delete_symbol",
    description: "Delete one function/class/method/interface/type by exact symbol name using tree-sitter. Supports TS/TSX/JS/JSX/Python/Go/Rust/Java. Call read_file first. If the symbol is missing or ambiguous, no file is changed and candidates are returned.",
    parameters: {
      type: "object",
      properties: {
        path: { type: "string" },
    ...
    fn: async (args, ctx) => {
      ...
      if (ctx?.readTracker && !ctx.readTracker.hasRead(abs)) {
        throw new Error(
          `delete_symbol: ${displayRel3(rootDir, abs)} was not read this session \u2014 read_file first so the AST deletion matches the bytes on disk.`
        );
      }
      if (!grammarForPath(abs)) {
        return JSON.stringify({
          path: args.path,
          error: "language not supported (TS/TSX/JS/JSX/Python/Go/Rust/Java); use edit_file instead"
        });
      }
      if (typeof args.name !== "string" || args.name.trim().length === 0) {
        return JSON.stringify({ path: args.path, error: "name must be a non-empty string" });
      }
      const { text } = decodeFileBuffer(await fs3.readFile(abs));
      const { extractSymbols } = await import("./symbols-UQ274IOB.js");
      const wantedKind = args.kind;
      const wantedParent = args.parent;
      const candidates = (await extractSymbols(abs, text)).filter(
        (symbol2) => symbol2.name === args.name && DELETE_SYMBOL_KINDS.has(symbol2.kind) && (!wantedKind || symbol2.kind === wantedKind) && (!wantedParent || symbol2.parent === wantedParent)
      );
      ...
      if (candidates.length > 1) {
        return JSON.stringify({
          ...
          candidates: candidates.map((symbol2) => ({
            name: symbol2.name,
            kind: symbol2.kind,
            line: symbol2.line,
            endLine: symbol2.endLine,
            parent: symbol2.parent
          }))
        });
      }
      ...
      if (symbol.line === symbol.endLine) {
        ...
        if (before.length > 0 || after.length > 0 && !after.startsWith("//")) {
          return JSON.stringify({
            path: args.path,
            error: "delete_symbol: target symbol shares a line with other code; use edit_file for precise removal"
          });
        }
      }
      ...
      return applyDeleteLineRange(
        ...
        symbol.endLine,
        "delete_symbol",
        ctx?.readTracker ? (p) => ctx.readTracker.hasRead(p) : void 0
      );
    }
  });
  registry.register({
  ...
  registry.register({
    ...
    fn: async (args, ctx) => {
      ...
      return `deleted ${displayRel3(rootDir, abs)}`;
    }
  });
  registry.register({
    ...
    fn: async (args, ctx) => {
      ...
      if (recursive) {
        await fs3.rm(abs, { recursive: true, force: false });
      } else {
        await fs3.rmdir(abs);
      }
      return `deleted ${displayRel3(rootDir, abs)}/${recursive ? " (recursive)" : ""}`;
    }
  });
  registry.register({
    name: "copy_file",
...
function renderTodos(todos) {
  ...
  const active = todos.filter((t2) => t2.status !== "completed");
  if (active.length === 0) return header;
  const lines = active.map((t2) => {
...
import { dirname as dirname4, isAbsolute as isAbsolute4, relative as relative8, resolve as resolve7 } from "path";
var BLOCK_RE = /^(\S[^\n]*)\n<{7} SEARCH\n([\s\S]*?)\n?={7}\n([\s\S]*?)\n?>{7} REPLACE/gm;
...
function resolveEditPath(rootDir, rawPath) {
  ...
  if (/^[A-Za-z]:[\\/]/.test(rawPath) || looksLikeAbsoluteSystemPath3(rawPath)) {
    return resolve7(rawPath);
  }
  let rooted = rawPath;
  while (rooted.startsWith("/") || rooted.startsWith("\\")) {
    rooted = rooted.slice(1);
  }
  return resolve7(absRoot, rooted || ".");
}
function looksLikeAbsoluteSystemPath3(rawPath) {
  return /^\/(?:home|Users|etc|var|opt|tmp|usr|mnt|Library|Volumes|proc|sys|dev|run|srv|media|Applications|System|root|boot|private)(?:[/\\]|$)/.test(
    rawPath
  );
}
...
function atomicReplaceFileSync(path, buf, mode) {
  ...
  try {
    ...
    fd = void 0;
    renameSync(tmp, path);
    fsyncDirectoryBestEffort(dirname4(path));
  } catch (err) {
    if (fd !== void 0) {
      try {
        closeSync(fd);
...
function applyEditBlock(block, rootDir) {
  ...
  if (!pathIsUnder3(absTarget, absRoot)) {
    return {
      path: block.path,
      status: "path-escape",
  ...
  if (searchEmpty) {
    try {
      mkdirSync2(dirname4(absTarget), { recursive: true });
...
function snapshotBeforeEdits(blocks, rootDir) {
  ...
  for (const b of blocks) {
    ...
    if (!pathIsUnder3(abs, absRoot)) continue;
  ...
  return snapshots.map((snap) => {
    ...
    try {
      if (snap.prevContent === null) {
        if (existsSync5(abs)) unlinkSync(abs);
        return {
          path: snap.path,
          status: "applied",
      ...
      writeFileSync2(abs, encodeFile(snap.prevContent, snap.prevEncoding ?? "utf8"));
      return {
        path: snap.path,
        status: "applied",
...
export {
  ...
  looksLikeAbsoluteSystemPath2 as looksLikeAbsoluteSystemPath,
  pathIsUnder2 as pathIsUnder,
...
*/
//# sourceMappingURL=chunk-4XMNMGWP.js.map