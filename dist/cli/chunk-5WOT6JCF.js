...
var SkillStore = class {
  ...
  readEntry(dir, scope, entry) {
    const isDir2 = entry.isDirectory() || entry.isSymbolicLink() && this.isSymlinkDirectory(dir, entry.name);
    const isFile = entry.isFile() || entry.isSymbolicLink() && !isDir2 && this.isSymlinkFile(dir, entry.name);
    if (isDir2) {
  ...
  isSymlinkDirectory(parentDir, name) {
    try {
      return statSync2(join2(parentDir, name)).isDirectory();
    } catch {
      return false;
    }
  }
  /** Check if a symlink points to a file. Returns false for broken symlinks. */
  isSymlinkFile(parentDir, name) {
    try {
      return statSync2(join2(parentDir, name)).isFile();
    } catch {
      return false;
    }
  }
...
function loadBodyWithReferences(skillFilePath, body) {
  if (!skillFilePath.endsWith(SKILL_FILE)) return body;
  const refsDir = join2(dirname(skillFilePath), "references");
  if (!existsSync2(refsDir)) return body;
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    let content;
    try {
      content = readFileSync2(join2(refsDir, entry), "utf8").trim();
    } catch {
      continue;
    }
...
function builtinSkillDescription(name) {
...
var BUILTIN_QQ_BODY = `Help the user configure or troubleshoot the built-in QQ channel in Reasonix. This skill is INLINED on purpose \u2014 stay in the parent loop and keep the guidance short.
...
var BUILTIN_SKILLS = Object.freeze([
  ...
  Object.freeze({
    name: "qq",
    description: "Guide QQ channel setup and troubleshooting for CLI or desktop. Best for: first-time setup, QQ connection failures, desktop QQ not replying, App ID / App Secret / QQ environment questions, and current-session routing behavior. Inlined \u2014 use when the user clearly needs help configuring or fixing the QQ channel.",
...
//# sourceMappingURL=chunk-5WOT6JCF.js.map