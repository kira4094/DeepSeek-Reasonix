...
function stripCompactionMarker(text) {
  return text.startsWith(COMPACTION_SUMMARY_MARKER) ? text.slice(COMPACTION_SUMMARY_MARKER.length) : text;
}
...
import { constants, closeSync, lstatSync, openSync, realpathSync } from "fs";
...
function ensureUnderSandbox(path, sandboxRoot, target) {
...
function resolveSensitivePath(token, projectRoot) {
  ...
  if (expanded.startsWith("~")) {
    expanded = pathMod2.join(homedir2(), expanded.slice(1));
  }
  return pathMod2.resolve(projectRoot, expanded);
}
function expandPrefix(prefix) {
  if (prefix.startsWith("~")) return pathMod2.join(homedir2(), prefix.slice(1));
...
function pathIsUnder2(child, parent) {
  const rel = pathMod2.relative(parent, child);
  return rel === "" || !rel.startsWith("..") && !pathMod2.isAbsolute(rel);
}
...
function redirectsEscapeSandbox(chain, projectRoot) {
  const root = pathMod2.resolve(projectRoot);
  for (const target of redirectTargets(chain)) {
    const resolved = pathMod2.resolve(root, target);
    if (!pathIsUnder2(resolved, root)) return true;
  }
  return false;
}
  ...
  for (const prefix of allowlist) {
    ...
    if (projectRoot) {
      ...
      for (const tok of argv) {
        ...
        let expanded = tok;
        if (expanded.startsWith("~")) expanded = pathMod2.join(homedir2(), expanded.slice(1));
        const resolved = pathMod2.resolve(root, expanded);
        if (pathMod2.isAbsolute(resolved) && !pathIsUnder2(resolved, root)) {
          return false;
        }
      }
    }
...
function encodeFile(text, encoding) {
  ...
  return import_iconv_lite.default.encode(text, "gb18030");
}
...
function computeDeleteRangePatchFromText(text, args) {
  ...
  if (startHits.length !== 1) {
    return noDeletePatch(
      startHits.length === 0 ? "start_anchor not found" : `start_anchor appears ${startHits.length} times`
    );
  }
...
function computeDeleteLineRangePatchFromText(text, startLine, endLine) {
...
async function applyDeleteRange(rootDir, abs, args, hasRead) {
  if (hasRead && !hasRead(abs)) {
    throw new Error(
...
async function applyDeleteLineRange(rootDir, abs, startLine, endLine, toolName, hasRead) {
  if (hasRead && !hasRead(abs)) {
    throw new Error(
...
function expandSymbolDeletionStartLine(lines, symbolLine) {
...
function expandDecoratorStartLine(lines, startLine) {
...
function looksLikeDecoratorContinuation(trimmed) {
  return /^[)\]}]/.test(trimmed);
}
function findDecoratorStartAbove(lines, lastLine) {
...
function expandDocCommentStartLine(lines, startLine) {
...
var WRITE_DIFF_MAX_LINES = 5e3;
var WRITE_DIFF_MAX_BYTES = 100 * 1024;
function generateWriteDiff(oldContent, newContent, rel) {
  ...
  if (oldContent.length > WRITE_DIFF_MAX_BYTES || newContent.length > WRITE_DIFF_MAX_BYTES || oldContent.split(/\r?\n/).length > WRITE_DIFF_MAX_LINES || newContent.split(/\r?\n/).length > WRITE_DIFF_MAX_LINES) {
...
//# sourceMappingURL=chunk-75N64M56.js.map