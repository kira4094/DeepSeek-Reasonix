  ...
  "node_modules/domutils/lib/helpers.js"(exports) {
    ...
    function uniqueSort(nodes) {
      ...
      nodes.sort(function(a, b) {
        var relative8 = compareDocumentPosition(a, b);
...
import { dirname, join as join3, relative as relative3, resolve as resolve4 } from "path";
function findDirMemory(absDir, rootDir) {
  const root = resolve4(rootDir);
  const target = resolve4(absDir);
...
function findSubdirMemoryAncestors(absPath, rootDir) {
  return findDirMemory(dirname(resolve4(absPath)), rootDir);
}
...
var RegexRunner = class {
  ...
  testLines(text, source, flags, opts = {}) {
    return new Promise((resolve7, reject) => {
      ...
      const entry = { resolve: resolve7, reject, timer };
      if (opts.signal) {
...
import { dirname as dirname3, isAbsolute as isAbsolute3, relative as relative7, resolve as resolve6 } from "path";
var BLOCK_RE = /^(\S[^\n]*)\n<{7} SEARCH\n([\s\S]*?)\n?={7}\n([\s\S]*?)\n?>{7} REPLACE/gm;
...
function resolveEditPath(rootDir, rawPath) {
  ...
  return resolve6(absRoot, rooted || ".");
}
...
function atomicReplaceFileSync(path, buf, mode) {
  ...
  try {
    ...
    fd = void 0;
    renameSync(tmp, path);
    fsyncDirectoryBestEffort(dirname3(path));
  } catch (err) {
    if (fd !== void 0) {
      try {
        closeSync(fd);
  ...
  return snapshots.map((snap) => {
    ...
    try {
      if (snap.prevContent === null) {
        if (existsSync4(abs)) unlinkSync(abs);
        return {
          path: snap.path,
          status: "applied",
...
*/
//# sourceMappingURL=chunk-BLRVVIQ2.js.map