...
// src/desktop/qq-ingress.ts
function classifyDesktopQQIngress(opts) {
  if (opts.hasPendingInteraction) return "pause_reply";
  if (opts.isBusy) return "busy";
  return "new_turn";
}

// src/desktop/qq-remote-commands.ts
function parseQQRemoteDesktopCommand(text, skillNames) {
  ...
  if (modelMatch) {
    ...
    return { kind: "model", value: value || void 0 };
  }
  ...
  if (planMatch) {
    const value = planMatch[1]?.trim().toLowerCase();
  ...
  if (rawName === "help" || rawName === "new" || rawName === "abort" || rawName === "compact" || rawName === "retry" || rawName === "model" || rawName === "effort" || rawName === "plan") {
    return null;
  }
  const names = new Set(skillNames);
  if (!names.has(rawName)) return null;
  const args = rawArgs?.trim() ?? "";
  return { kind: "skill", name: rawName, args: args || void 0 };
}
function qqRemoteDesktopHelpText(skillNames) {
...
function qqRemoteCommandBypassesBusy(cmd) {
...
function hasQQPendingInteraction(state, tabId) {
...
function normalizeImportedMcpServer(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const raw = value;
  const name = typeof raw.name === "string" ? raw.name.trim() : "";
  if (!name) return null;
  const cwd = typeof raw.cwd === "string" && raw.cwd.trim().length > 0 ? raw.cwd.trim() : void 0;
  ...
  if (transport === "stdio") {
    ...
    return {
      ...
      config: {
        ...
        cwd,
        disabled: raw.disabled === true ? true : void 0,
        requestTimeoutMs: typeof raw.requestTimeoutMs === "number" && Number.isFinite(raw.requestTimeoutMs) ? raw.requestTimeoutMs : void 0
      }
    };
  }
...
function rawForImportedMcpServer(name, config) {
  if (config.transport === "stdio") {
    if (!config.command) return void 0;
    return specToRaw({
      name,
      transport: "stdio",
...
function importedMcpServerFromSpec(spec) {
  if (!spec.name) return void 0;
  const base = {
    name: spec.name,
    transport: spec.transport,
    disabled: spec.disabled === true ? true : void 0,
...
function stripLegacyMcpConfigForNames(cfg, names) {
  ...
  if (Array.isArray(cfg.mcp) && cfg.mcp.length > 0) {
    ...
    if (cfg.mcp.length === 0) cfg.mcp = void 0;
  }
  if (Array.isArray(cfg.mcpDisabled) && cfg.mcpDisabled.length > 0) {
    cfg.mcpDisabled = cfg.mcpDisabled.filter((name) => !names.has(name));
    if (cfg.mcpDisabled.length === 0) cfg.mcpDisabled = void 0;
  }
  if (cfg.mcpEnv) {
    for (const name of names) delete cfg.mcpEnv[name];
    if (Object.keys(cfg.mcpEnv).length === 0) cfg.mcpEnv = void 0;
  }
}
function legacyMcpRawMatches(entry, target) {
...
function stripLegacyMcpConfigForRaw(cfg, raw) {
  if (!Array.isArray(cfg.mcp) || cfg.mcp.length === 0) return;
  cfg.mcp = cfg.mcp.filter((entry) => !legacyMcpRawMatches(entry, raw));
  if (cfg.mcp.length === 0) cfg.mcp = void 0;
}
function importedMcpRawVariants(name, config) {
  const variants = [rawForImportedMcpServer(name, config), rawForImportedMcpServer(null, config)];
  return [...new Set(variants.filter((raw) => typeof raw === "string"))];
}
function applyImportedMcpServersToConfig(cfg, servers) {
  const canonical = { ...cfg.mcpServers ?? {} };
  const importedNames = /* @__PURE__ */ new Set();
  const forceSpecs = /* @__PURE__ */ new Set();
  const legacyRawSpecs = /* @__PURE__ */ new Set();
...
function applyMcpSpecUpdateToConfig(cfg, raw, server) {
  const normalized = normalizeImportedMcpServer(server);
  if (!normalized) {
  ...
  let oldName;
  try {
    oldName = parseMcpSpec(raw).name ?? void 0;
  } catch {
    oldName = void 0;
  }
  ...
  stripLegacyMcpConfigForNames(cfg, /* @__PURE__ */ new Set([normalized.name, ...oldName ? [oldName] : []]));
  ...
  if (Object.keys(cfg.mcpServers).length === 0) cfg.mcpServers = void 0;
  const updatedRaw = rawForImportedMcpServer(normalized.name, normalized.config);
  return { updatedRaw, forceSpecs: [.../* @__PURE__ */ new Set([raw, ...updatedRaw ? [updatedRaw] : []])] };
}
...
var LOADED_RECENT_MESSAGE_WINDOW = 120;
...
function classifyMcpStatusReason(reason) {
  if (!reason) return void 0;
  const lower = reason.toLowerCase();
  if (lower.includes("no bearer token") || lower.includes("missing bearer")) {
    return "missing-token";
  }
  if (lower.includes("401") || lower.includes("unauthorized") || lower.includes("invalid_token") || lower.includes("authentication required") || lower.includes("forbidden") || lower.includes("403")) {
    return "auth";
  }
  if (lower.includes("enoent") || lower.includes("command not found") || lower.includes("not found in path") || lower.includes("spawn")) {
    return "command";
  }
  if (lower.includes("timeout") || lower.includes("econn") || lower.includes("network") || lower.includes("dns") || lower.includes("fetch failed")) {
    return "network";
  }
  return "unknown";
}
function mcpToolsForSummary(summary) {
...
function emitMcpSpecs(tab) {
  ...
  const specs = allSpecs.map((spec) => {
    ...
    return {
      ...withConfig,
      status: live.kind,
      statusHint: live.kind === "failed" ? classifyMcpStatusReason(live.reason) : void 0,
...
`
  ...
  function sendQQInfo(message, tabOverride) {
  ...
  function emitQQNotice(message, tabOverride) {
    const tab = tabOverride ?? activeDesktopTab();
    if (tab) {
      emit(
        {
  ...
  function startNewChatInTab(tab) {
    if (tab.aborter) tab.switching = true;
    abortTurn(tab);
  ...
  function buildSkillPayload(tab, name, args) {
    const store = new SkillStore({
      projectRoot: tab.rootDir,
      customSkillPaths: loadResolvedSkillPaths(tab.rootDir)
    });
...
${found.body}${argsLine}`;
  ...
  function availableSkillNamesForTab(tab) {
    const store = new SkillStore({
      projectRoot: tab.rootDir,
      customSkillPaths: loadResolvedSkillPaths(tab.rootDir),
      subagentModels: loadSubagentModels()
  ...
  function runBtwOnTab(tab, question, hooks) {
    if (!tab.runtime) return;
    void (async () => {
      try {
        ...
        const message = `/btw failed: ${err.message}`;
        emit({ type: "$error", message }, tab.id);
        hooks?.onError?.(message);
      }
    })();
  }
  function handleQQRemoteDesktopCommand(tab, text) {
    const cmd = parseQQRemoteDesktopCommand(text, availableSkillNamesForTab(tab));
    if (!cmd) return false;
    if (tab.aborter && !qqRemoteCommandBypassesBusy(cmd)) {
      void qqRuntime.channel?.sendResponse("Session is busy. Wait for the current turn or reply to the pending prompt.").catch(() => void 0);
      return true;
    }
    switch (cmd.kind) {
      case "help":
      ...
      case "compact":
        ...
        void tab.runtime.loop.compactHistory().then(() => {
          emitCtxBreakdown(tab);
          ...
          void qqRuntime.channel?.sendResponse(`/compact failed: ${err.message}`).catch(() => void 0);
        });
        return true;
      case "retry": {
        if (!tab.runtime) {
      ...
      case "btw":
        ...
        runBtwOnTab(tab, cmd.text, {
          onAnswer: (answer) => void qqRuntime.channel?.sendResponse(`\u226B btw
${answer}`).catch(() => void 0),
          onError: (message) => void qqRuntime.channel?.sendResponse(message).catch(() => void 0)
        });
        return true;
      case "skill": {
        if (!tab.runtime) {
        ...
        if (!payload) {
          ...
          void qqRuntime.channel?.sendResponse(`skill not found: ${cmd.name}`).catch(() => void 0);
          return true;
        }
  ...
  function normalizeQQRemoteModel(value) {
  ...
  function applyDesktopModel(tab, next) {
...
${optionsList}${p.allowCustom ? "\n\n(You can also reply with custom text.)" : ""}`;
  ...
  function bridgeTabMcp(tab, opts2 = {}) {
    if (!tab.runtime || !tab.toolset) return Promise.resolve();
    if (tab.mcpRuntime) {
      return tab.mcpRuntime.reloadFromConfig(tab.runtime.loop, { force: opts2.forceSpecs }).then(() => emitMcpSpecs(tab)).catch((err) => {
...
`
      ...
      try {
        ...
        try {
          ...
          if (parsed.name) {
            ...
            stripLegacyMcpConfigForNames(cfg, /* @__PURE__ */ new Set([parsed.name]));
          }
        } catch {
        }
    ...
    if (msg.cmd === "mcp_import_servers") {
      try {
        ...
        emit({ type: "$error", message: `mcp_import_servers: ${err.message}` }, tab.id);
      }
      return;
    }
    if (msg.cmd === "mcp_specs_update") {
      try {
        ...
        emit({ type: "$error", message: `mcp_specs_update: ${err.message}` }, tab.id);
      }
      return;
    }
    if (msg.cmd === "mcp_specs_retry") {
      try {
        ...
        emit({ type: "$error", message: `mcp_specs_retry: ${err.message}` }, tab.id);
      }
      return;
    }
...
`);
    ...
    if (msg.cmd === "settings_save") {
      try {
        ...
        if (msg.recentWorkspaces !== void 0) {
          const cfg = readConfig();
          cfg.recentWorkspaces = msg.recentWorkspaces;
          writeConfig(cfg);
        }
        if (msg.editor !== void 0) saveEditor(msg.editor);
        if (msg.desktopCloseBehavior === "closeToTray" || msg.desktopCloseBehavior === "closeToQuit") {
        ...
        if (msg.webSearchEngine !== void 0 || msg.webSearchEndpoint !== void 0 || msg.metasoApiKey !== void 0 || msg.baiduApiKey !== void 0 || msg.tavilyApiKey !== void 0 || msg.perplexityApiKey !== void 0 || msg.exaApiKey !== void 0 || msg.ollamaApiKey !== void 0 || msg.braveApiKey !== void 0) {
          const cfg = readConfig();
          ...
          if (msg.baiduApiKey !== void 0) {
            cfg.baiduApiKey = msg.baiduApiKey?.trim() || void 0;
          }
    ...
    if (msg.cmd === "prompt_history_step") {
      try {
        ...
        emit(
          { type: "$error", message: `prompt_history_step failed: ${err.message}` },
          tab.id
        );
      }
      return;
...
//# sourceMappingURL=desktop-O65TWSLB.js.map