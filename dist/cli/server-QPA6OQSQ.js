...
async function loadCachedText(path) {
...
async function loadCachedBinary(path) {
  const fd = await open(path, "r");
  try {
    const fileStat = await fd.stat();
...
async function loadChunk(name, token) {
  try {
...
async function loadVendorCss(name, token) {
  try {
...
async function loadDistFile(name) {
...
async function serveAsset(name, token = "") {
  if (name === "app.js") {
...
async function readSubdirs(path) {
  let names;
  try {
...
async function handleBrowse(method, _rest, _body, _ctx, query) {
  ...
  if (!rawPath) {
    ...
    if (isWin) {
      const drives = (await listWindowsDrives()).filter((d) => resolve(d) !== resolve(home)).map((d) => ({ name: d, full: d }));
...
async function handleFileReadPreview(method, _rest, _body, ctx, query = new URLSearchParams()) {
  if (method !== "GET") return { status: 405, body: { error: "GET only" } };
...
async function handleFiles(method, _rest, body, ctx, query = new URLSearchParams()) {
  if (method === "GET" && _rest[0] === "search") {
    ...
    try {
      if (!(await stat2(cwd2)).isDirectory()) {
        return { status: 503, body: { error: "@-mention picker requires a code-mode session" } };
      }
    } catch {
      return { status: 503, body: { error: "@-mention picker requires a code-mode session" } };
    }
    const prefix2 = (query.get("q") ?? "").trim().toLowerCase();
    const matches2 = await walk(cwd2, prefix2);
    return {
      status: 200,
      body: { nonce: query.get("nonce"), query: query.get("q") ?? "", results: matches2 }
    };
  }
  if (method !== "POST") return { status: 405, body: { error: "POST only" } };
  const cwd = ctx.getCurrentCwd?.();
    ...
    for (const name of entries) {
      ...
      try {
        ...
        if (s.isFile()) {
          ...
          try {
            const inner = await readdir3(full);
            for (const child of inner) {
              try {
                const cs = await stat3(join4(full, child));
                if (cs.isFile()) {
                  fileCount++;
                  totalBytes += cs.size;
                }
...
async function handleHealth(method, _rest, _body, ctx) {
  ...
  const [sessionsStat, memoryStat, semanticStat] = await Promise.all([
    dirSize(join4(reasonixHome, "sessions")),
    dirSize(join4(reasonixHome, "memory")),
    dirSize(join4(reasonixHome, "semantic"))
  ]);
  let usageBytes = 0;
  try {
    usageBytes = (await stat3(ctx.usageLogPath)).size;
  } catch {
  }
...
async function writeSettingsFile(path, hooksBlock) {
...
function handleGetProjects() {
  const projects = {};
  if (!existsSync2(MEM_SESSIONS)) return { status: 200, body: projects };
          ...
          for (const date of dates) {
            ...
            for (const sessionDir of readdirSync(dateDir)) {
              ...
              for (const ch of chunks) {
                try {
                  const raw = readFileSync2(join5(sDir, ch), "utf8");
...
function handleGetSessions(hash) {
  const hashDir = join5(MEM_SESSIONS, hash);
  if (!existsSync2(hashDir)) return { status: 200, body: [] };
  const sessions = [];
    ...
    for (const sessionDir of readdirSync(dateDir)) {
      ...
      for (const chunk of chunks) {
        try {
          ...
          const raw = readFileSync2(cp, "utf8");
...
function handleGetSession(hash, dateRaw, sessionDir) {
  ...
  if (!existsSync2(dateDir)) return { status: 404, body: { error: "Session not found" } };
...
import { mkdir as mkdir2, readFile as readFile3, readdir as readdir4, stat as stat4, unlink, writeFile as writeFile2 } from "fs/promises";
...
async function listMemoryFiles(dir) {
  try {
    ...
    const stats = await Promise.all(
      mdFiles.map(async (f) => {
        const s = await stat4(join6(dir, f));
        return {
          name: f.replace(/\.md$/, ""),
          size: s.size,
...
async function handleMemory(method, rest, body, ctx, query = new URLSearchParams()) {
  ...
  if (method === "GET") {
    if (scope === "project") {
      ...
      return { status: 200, body: { path, body: await readFile3(path, "utf8") } };
    }
    if ((scope === "global" || scope === "project-mem") && name && SAFE_NAME.test(name)) {
      ...
      try {
        const body2 = await readFile3(path, "utf8");
        return { status: 200, body: { path, body: body2 } };
      } catch {
...
async function handleProjectTree(method, _rest, _body, ctx) {
  ...
  try {
    if (!(await stat5(cwd)).isDirectory()) {
      return { status: 503, body: { error: "no project directory available" } };
    }
  } catch {
    return { status: 503, body: { error: "no project directory available" } };
...
async function buildTree(root, dirPath, depth) {
  if (depth > MAX_DEPTH2) return [];
  let names;
  try {
    names = await readdir5(dirPath);
  } catch {
    return [];
  }
  const nodes = [];
...
async function readIndexMeta2(root, current) {
  ...
  try {
    ...
    try {
      ...
      sizeBytes = stat8.size;
      const buf = Buffer.alloc(stat8.size);
      let read = 0;
      while (read < stat8.size) {
        const n = readSync2(fd, buf, read, stat8.size - read, read);
        if (n <= 0) break;
        read += n;
      }
...
import { readFile as readFile4, stat as stat6 } from "fs/promises";
async function parseTranscript(path, maxBytes = 4 * 1024 * 1024) {
  let raw;
  try {
...
async function handleSettings(method, _rest, body, ctx) {
  if (method === "GET") {
    ...
    return {
      ...
      body: {
        ...
        webSearchApiKeys: {
          baidu: baiduApiKey ? redactKey(baiduApiKey) : void 0
        },
  ...
  if (method === "POST") {
    ...
    if (fields.webSearchEngine !== void 0) {
      if (typeof fields.webSearchEngine !== "string" || !VALID_WEB_SEARCH_ENGINES.has(fields.webSearchEngine)) {
        return {
          status: 400,
          body: {
            error: "webSearchEngine must be bing | bing-intl | searxng | metaso | baidu | tavily | perplexity | exa | brave | ollama"
          }
        };
      }
    ...
    if (fields.baiduApiKey !== void 0) {
      ...
      cfg.baiduApiKey = trimmed.length > 0 ? trimmed : void 0;
...
import { mkdir as mkdir3, open as open2, readFile as readFile5, readdir as readdir6, rm, stat as stat7, writeFile as writeFile3 } from "fs/promises";
...
async function readSkillListEntry(skillPath, name, scope) {
  const fd = await open2(skillPath, "r");
  try {
    const fileStat = await fd.stat();
...
async function resolveSkillPath(dir, name) {
  const folderPath = join9(dir, name, SKILL_FILE);
  try {
    if ((await stat7(folderPath)).isFile()) return { path: folderPath, layout: "folder" };
  } catch {
  }
  const flatPath = join9(dir, `${name}.md`);
  try {
    if ((await stat7(flatPath)).isFile()) return { path: flatPath, layout: "flat" };
  } catch {
  }
  return null;
}
...
async function listSkills(dir, scope) {
  const out = [];
  try {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        if (!SAFE_NAME2.test(entry.name)) continue;
        candidates.push({ skillPath: join9(dir, entry.name, SKILL_FILE), name: entry.name });
      } else if (entry.isFile() && entry.name.endsWith(".md")) {
        const name = entry.name.slice(0, -3);
        if (!SAFE_NAME2.test(name)) continue;
        candidates.push({ skillPath: join9(dir, entry.name), name });
      }
    }
...
async function handleSkills(method, rest, body, ctx) {
  ...
  if (method === "GET") {
    ...
    return {
      status: 200,
      body: { path: resolved.path, body: await readFile5(resolved.path, "utf8") }
    };
  }
  if (method === "POST") {
    ...
    await mkdir3(dirname5(target.path), { recursive: true });
    await writeFile3(target.path, contents, "utf8");
    ctx.audit?.({
      ts: Date.now(),
      action: "save-skill",
  ...
  if (method === "DELETE") {
    if (!resolved) return { status: 404, body: { error: "skill not found" } };
    await rm(resolved.layout === "folder" ? dirname5(resolved.path) : resolved.path, {
      recursive: true,
      force: true
    });
...
async function dispatch(req, res, ctx, expectedToken) {
  ...
  if (path === "/mem" || path === "/mem/index.html") {
    ...
    if (existsSync4(memUiPath)) {
      const html = readFileSync3(memUiPath, "utf8");
...
//# sourceMappingURL=server-QPA6OQSQ.js.map